                                    ...
                                    <tr>
                                        ...
                                        <td data-label="Действия" class="text-end">
                                            <form method="post" action="{{ route('deposit.destroy', $deposit->deposit_id) }}">
                                                ...
                                                <a title="Открыть" href="{{ route('deposit.show', $deposit->deposit_id) }}" class="btn btn-sm btn-primary me-1"><i class="bi bi-text-center" style="font-size: 1rem"></i></a>
                                                <a title="Изменить" href="{{ route('deposit.edit', $deposit->deposit_id) }}" class="btn btn-sm btn-warning me-1"><i class="bi bi-pencil-square" style="font-size: 1rem"></i></a>
                                                <button type="submit" title="Удалить" onclick="return confirm('Вы уверены, что хотите удалить вклад?')" class="btn btn-sm btn-danger"><i class="bi bi-trash" style="font-size: 1rem"></i></button>
                                            </form>
                                        </td>
                                    </tr>
                                @empty
