                                            ...
                                            <tbody>
                                                @forelse($wallet['payments'] as $payment)
                                                    <tr>
                                                        ...
                                                        <td data-label="{{ __('Баланс') }}" class="text-end">{{ number_format($payment['inset_balance'], 0, '.', ' ') }} {{ $wallet['currency']['abbr'] }}</td>
                                                        <td data-label="{{ __('Прирост') }}" class="text-end {{ $payment['transaction_amount'] > 0 ? 'text-success': 'text-danger' }}">
                                                            {{ number_format($payment['transaction_amount'], 0, '.', ' ') }} {{ $wallet['currency']['abbr'] }}
                                                        </td>
                                                        <td data-label="{{ __('Комментарий') }}" class="text-center">{{ $payment['note'] }}</td>
                                                        <td data-label="{{ __('Остаток') }}" class="text-end">{{ number_format($payment['outset_balance'], 0, '.', ' ') }} {{ $wallet['currency']['abbr'] }} <i class="{{ $payment['transaction_amount'] > 0 ? 'text-success ' . Icons::PROFIT : 'text-danger ' . Icons::LOSS }}"></i></td>
                                                        <td class="text-end" data-label="{{ __('Действия') }}">
                                                            <form method="post" action="{{ route('wallet.payment.destroy', $payment['payment_id']) }}">
                                                                @csrf
                                                                @method('DELETE')

                                                                <a title="{{ __('Изменить') }}" href="{{ route('wallet.payment.edit', $payment['payment_id']) }}" class="btn btn-warning"><i class="bi bi-pencil-square" style="font-size: 1rem"></i></a>
                                                                <button type="submit" title="{{ __('Удалить') }}" onclick="return confirm('{{ __('Вы уверены, что хотите удалить транзакцию?') }}')" class="btn btn-danger"><i class="bi bi-trash" style="font-size: 1rem"></i></button>
                                                            </form>
                                                        </td>
                                                    </tr>
                                                @empty
